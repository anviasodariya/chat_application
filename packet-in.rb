...
class PacketinDumper < Controller
 add_timer_event :age_fdb, 5, :periodic
  ...
  def packet_in(datapath_id, event)
    if ((event.ipv4_src == "192.168.0.1" && event.ipv4_dst == "192.168.0.2") || (event.ipv4_src == "192.168.0.2" && event.ipv4_dst == "192.168.0.1") || (event.ipv4_src == "192.168.0.1" && event.ipv4_dst == "192.168.0.3") || (event.ipv4_src == "192.168.0.3" && event.ipv4_dst == "192.168.0.1") || (event.ipv4_src == "192.168.0.2" && event.ipv4_dst == "192.168.0.3") || (event.ipv4_src == "192.168.0.3" && event.ipv4_dst == "192.168.0.2"))

    @fdb.learn event.eth_src, event.in_port
    port_no = @fdb.port_no_of(event.eth_dst)
    if port_no
      flow_mod datapath_id, event, port_no
      packet_out datapath_id, event, port_no
    else
      flood datapath_id, event
    end
